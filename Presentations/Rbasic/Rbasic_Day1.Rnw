...
\setlength{\fancyvrbpartopsep}{-1pt}
<<>>=
df %>% group_by(sex) %>% filter(sex == "female") %>% top_n(1, weight)
@
\rule{\textwidth}{0.4pt}\\
\small
		\item Which is the minimum male weight? 
...
\setlength{\fancyvrbpartopsep}{-1pt}
<<>>=
df %>% group_by(sex) %>% filter(sex == "male") %>% filter(weight == min(weight))
@
\rule{\textwidth}{0.4pt}\\
\small
\end{itemize}
...
\setlength{\fancyvrbpartopsep}{-1pt}
<<>>=
df %>% group_by(sex) %>% filter(sex == "male") %>% filter(height > 180) %>% summarise(mean(age))
@
\rule{\textwidth}{0.4pt}\\
\small
		\item Who is the younger and lighter female? 
...
\setlength{\fancyvrbpartopsep}{-1pt}
<<>>=
df %>% group_by(sex) %>% filter(sex == "female") %>% filter(age == min(age)) %>% filter(weight == min(weight))
@
\rule{\textwidth}{0.4pt}\\
\small
